...
#ifndef Step2Analyzer_h
#define Step2Analyzer_h
...
//for debug
void BREAK(char* info=""){
	cout<<info<<"!!!  entry a char to continue"<<endl;
	char tmp;cin>>tmp;
}
...
class Step2Analyzer {
	public :
		...
		Step2Analyzer(TTree *tree, TString inPlotDir);
		virtual ~Step2Analyzer();
...
#ifdef Step2Analyzer_cxx
Step2Analyzer::Step2Analyzer(TTree *tree, TString inPlotDir) : fChain(0) 
{
	/*
...
Step2Analyzer::~Step2Analyzer()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t Step2Analyzer::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t Step2Analyzer::LoadTree(Long64_t entry)
{
...
void Step2Analyzer::Init(TTree *tree)
{
	...
	/*	fChain->SetBranchAddress("METnoPU", &METnoPU_et, &b_METnoPU);
		...
		*/	
	fChain->SetBranchAddress("METnoPU", &METnoPU, &b_METnoPU);
	fChain->SetBranchAddress("METnoPUCh", &METnoPUCh, &b_METnoPUCh);
	fChain->SetBranchAddress("MET", &MET, &b_MET);
	fChain->SetBranchAddress("METtype1corr", &METtype1corr, &b_METtype1corr);
	fChain->SetBranchAddress("METtype1p2corr", &METtype1p2corr, &b_METtype1p2corr);
	fChain->SetBranchAddress("METnoPUtype1corr", &METnoPUtype1corr, &b_METnoPUtype1corr);
	fChain->SetBranchAddress("METnoPUtype1p2corr", &METnoPUtype1p2corr, &b_METnoPUtype1p2corr);
	fChain->SetBranchAddress("METtype1diff", &METtype1diff, &b_METtype1diff);
	fChain->SetBranchAddress("fakeMET", &fakeMET, &b_fakeMET);
...
void Step2Analyzer::Show(Long64_t entry)
{
...
Int_t Step2Analyzer::Cut(Long64_t entry)
{
...
//	if( !((Vtype==0||Vtype==1) && V.pt>130 && hJet_pt[0]>30 && hJet_pt[1]>30 ) ) return -1;
//
    // Higgs and 2 b-jets
...
//    if( !( abs(hJet_eta[0])<2.5 && abs(hJet_eta[1])<2.5 && hJet_id[0]==1 && hJet_id[1]==1 && hJet_puJetIdL[0]>0 && hJet_puJetIdL[1]>0 && hJet_csv[0]>0 && hJet_csv[1]>0  ) ) return -1;
	...
	if(!(nvlep==2)) return -1;
	//if( vLepton_mass[0]<-1  || vLepton_mass[1]<-1 ) return -1;
	if( !(vLepton_type[1]==13 && vLepton_type[0]==13 && vLepton_charge[0]*vLepton_charge[1]<0 ) ) return -1;// 13 is muon; 11 is ele
    if( !(vLepton_pt[0]>20 && vLepton_pt[1]>20) ) return -1;
	if( !(abs(vLepton_eta[0])<2.5 && abs(vLepton_eta[1])<2.5) ) return -1;
