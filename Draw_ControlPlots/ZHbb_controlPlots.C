...
TH1D* GetHist(TString var_name, TString fileName, Int_t fillcolor=1, Int_t linecolor=1);
...
void ZHbb_controlPlots(){
    ...
    for(int i = 0; i < 9; i++){
        ...
        TH1D* hist_VV = GetHist(varName[i], dibosons_file, 5);
		//Single Top
        ...
        TH1D* hist_singleTop = GetHist(varName[i], singleTops_file,3 );
...
TH1D* GetHist(TString var_name, vector<TString> filesName, Int_t fillcolor, Int_t linecolor){
	TH1D* hist=  GetHist(var_name, filesName[0], fillcolor, linecolor);
	for(Int_t i=1;i<Int_t(filesName.size());i++ ){
		TH1D* hist_tmp=  GetHist(var_name, filesName[i], fillcolor, linecolor);
		(*hist)=(*hist)+(*hist_tmp);
	}
